...
class MediaCorePlugin(
    private val backend: MediaCoreBackend = createMediaCoreBackend(),
) : GraphynPlugin {
    ...
    override fun register(registrar: GraphynPluginRegistrar) {
        ...
        registrar.registerExecutor(
            MediaCoreSpecs.audiosList.type,
            mediaListExecutor("audio", "audios")
        )
        registrar.registerExecutor(
            MediaCoreSpecs.videosList.type,
            mediaListExecutor("video", "videos")
        )
        ...
        registrar.registerExecutor(
            MediaCompositionSpecs.imageImport.type,
            imageImportExecutor(backend)
        )
        registrar.registerExecutor(
            MediaCompositionSpecs.captionOverlay.type,
            captionOverlayExecutor(backend)
        )
        registrar.registerExecutor(
            MediaCompositionSpecs.videoCompose.type,
        ...
        registrar.registerExecutor(
            MediaCompositionSpecs.timingController.type,
            timingControllerExecutor()
        )
        registrar.registerExecutor(MediaBuilderSpecs.videoOverlay.type, videoOverlayExecutor())
        registrar.registerExecutor(
            MediaBuilderSpecs.overlaysList.type,
            recordListExecutor("overlay", "overlays", "Overlays List")
        )
        registrar.registerExecutor(MediaBuilderSpecs.syncPoint.type, syncPointExecutor())
        registrar.registerExecutor(
            MediaBuilderSpecs.syncPointsList.type,
            recordListExecutor("point", "sync_points", "Sync Points List")
        )
    ...
    private fun videoImportExecutor() = NodeExecutor { inputs ->
        val metadata = backend.inspectVideo(inputs.stringOrError("path"))
        mapOf(
    ...
    private fun audioMixExecutor() = NodeExecutor { inputs ->
        val audioPaths = inputs.listOrError("audio_tracks").map { MediaTypes.path(it, "audio") }
    ...
    private fun videoStitchExecutor() = NodeExecutor { inputs ->
        ...
        ) { "Phase 1 supports only cut transitions." }
        val videoPaths = inputs.listOrError("videos").map { MediaTypes.path(it, "video") }
