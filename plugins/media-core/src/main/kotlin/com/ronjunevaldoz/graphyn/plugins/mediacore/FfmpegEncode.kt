...
/** Mix/stitch/encode operations for [com.ronjunevaldoz.graphyn.plugins.mediacore.ffmpeg.FfmpegMediaCoreBackend]; see that class for process plumbing. */
internal suspend fun FfmpegMediaCoreBackend.mixAudioImpl(
...
internal suspend fun FfmpegMediaCoreBackend.encodeVideoImpl(
    ...
    require(
        output.name.endsWith(
            ".mp4",
            ignoreCase = true
        )
    ) { "Video Encode output_path must end in .mp4." }
    ...
    command += listOf(
        ...
        "libx264",
        "-b:v",
        bitrate.toFfmpegBitrate(),
        "-pix_fmt",
        "yuv420p"
    )
    if (audioPath != null) command += listOf(
        "-map",
        "1:a:0",
        "-c:a",
        "aac",
...
internal suspend fun FfmpegMediaCoreBackend.encodeAudioImpl(
    ...
    ) { "Audio Encode output_path for '$format' must end in .$ext." }
    output.parentFile?.mkdirs()
