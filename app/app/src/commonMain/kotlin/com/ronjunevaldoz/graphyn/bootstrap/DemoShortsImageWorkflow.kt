...
internal val imageShortsWorkflow = WorkflowDefinition(
    ...
    nodes = buildList {
        ...
        repeat(SHORTS_SCENE_COUNT) { index ->
            val scene = index + 1
            add(NodeRef("scene$scene", SHORTS_SCENE_SUBGRAPH_NODE_TYPE, subgraph = imageSceneSubgraph(scene)))
        }
        ...
        add(NodeRef("encode", "media.video_encode", config = mapOf("output_path" to s("ai-shorts-image.mp4"), "bitrate" to s("high"), "codec" to s("h264"))))
