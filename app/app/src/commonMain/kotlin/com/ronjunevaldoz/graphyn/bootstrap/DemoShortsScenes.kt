...
private fun sceneSubgraph(index: Int, useImageMotion: Boolean) = WorkflowDefinition(
    ...
    nodes = buildList {
        add(NodeRef("diffusion", "sd.diffusion", config = mapOf("diffusion_model_path" to s(WAN5B_DIFFUSION))))
        add(NodeRef("encoders", "sd.encoders", config = mapOf("t5xxl_path" to s(WAN5B_T5), "clip_vision_path" to s(WAN5B_CLIP_VISION))))
        add(NodeRef("vae", "sd.vae", config = mapOf("vae_path" to s(WAN5B_VAE))))
        add(NodeRef("model", "sd.model"))
        add(NodeRef("ctx", "sd.context", config = mapOf("diffusion_flash_attn" to b(true), "n_threads" to i(-1))))
        add(NodeRef("sampler", "sd.sampler", config = mapOf("sample_method" to s("euler"), "scheduler" to s("discrete"), "sample_steps" to i(20), "txt_cfg" to d(5.0), "flow_shift" to d(5.0))))
        if (useImageMotion) {
            ...
            add(NodeRef("img2vid", "sd.img2vid", config = mapOf(
                "negative_prompt" to s(""),
                "width" to i(WIDE),
                "height" to i(TALL),
    ...
    connections = buildList {
        ...
        if (useImageMotion) {
            ...
            add(ConnectionRef("txt2img", "image", "img2vid", "init_image"))
            add(ConnectionRef("img2vid", "frames", "wrap", "input"))
        } else {
