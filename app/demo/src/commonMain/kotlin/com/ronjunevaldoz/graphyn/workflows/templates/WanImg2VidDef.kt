...
private const val WAN_INIT_IMAGE    = "../../app/demo/src/commonMain/resources/media/input.png"

/**
 * Wan2.2 image-to-video 720P preset using the A14B MoE model family.
 *
 * Wan2.2 A14B is the family that officially supports both 480P and 720P. This preset keeps the
 * higher-resolution size explicit so the launcher can offer a quality tier without changing the
 * model family or LoRA wiring.
 */
internal val wanImg2VidWorkflow = WorkflowDefinition(
    id = "wan-img2vid",
    name = "Wan Image to Video (720p)",
    nodes = listOf(
        guideNote(
            """
            Wan2.2 Image → Video · 720P preset

            Animates a still image using the A14B Wan2.2 MoE model family at
            720P. The same low-noise + high-noise LoRA pair is used here.
